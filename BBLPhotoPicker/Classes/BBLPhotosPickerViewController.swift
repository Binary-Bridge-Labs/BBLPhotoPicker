//
//  BBLPhotosPickerViewController.swift
//  BBLPhotosPicker
//
...
public protocol BBLPhotosPickerViewControllerDelegate: AnyObject {
    func dismissPhotoPicker(withPHAssets: [PHAsset])
    func dismissPhotoPicker(withBBLPHAssets: [BBLPHAsset])
    func shouldDismissPhotoPicker(withBBLPHAssets: [BBLPHAsset]) -> Bool
    func dismissComplete()
    func photoPickerDidCancel()
    func canSelectAsset(phAsset: PHAsset) -> Bool
    func didExceedMaximumNumberOfSelection(picker: BBLPhotosPickerViewController)
    func handleNoAlbumPermissions(picker: BBLPhotosPickerViewController)
    func handleNoCameraPermissions(picker: BBLPhotosPickerViewController)
}

extension BBLPhotosPickerViewControllerDelegate {
    public func deninedAuthoization() { }
    public func dismissPhotoPicker(withPHAssets: [PHAsset]) { }
    public func dismissPhotoPicker(withBBLPHAssets: [BBLPHAsset]) { }
    public func shouldDismissPhotoPicker(withBBLPHAssets: [BBLPHAsset]) -> Bool { return true }
    public func dismissComplete() { }
    public func photoPickerDidCancel() { }
    public func canSelectAsset(phAsset: PHAsset) -> Bool { return true }
    public func didExceedMaximumNumberOfSelection(picker: BBLPhotosPickerViewController) { }
    public func handleNoAlbumPermissions(picker: BBLPhotosPickerViewController) { }
    public func handleNoCameraPermissions(picker: BBLPhotosPickerViewController) { }
}

//for log
public protocol BBLPhotosPickerLogDelegate: AnyObject {
    func selectedCameraCell(picker: BBLPhotosPickerViewController)
    func deselectedPhoto(picker: BBLPhotosPickerViewController, at: Int)
    func selectedPhoto(picker: BBLPhotosPickerViewController, at: Int)
    func selectedAlbum(picker: BBLPhotosPickerViewController, title: String, at: Int)
}

extension BBLPhotosPickerLogDelegate {
    func selectedCameraCell(picker: BBLPhotosPickerViewController) { }
    func deselectedPhoto(picker: BBLPhotosPickerViewController, at: Int) { }
    func selectedPhoto(picker: BBLPhotosPickerViewController, at: Int) { }
    func selectedAlbum(picker: BBLPhotosPickerViewController, collections: [BBLAssetsCollection], at: Int) { }
}

public struct BBLPhotosPickerConfigure {
    ...
    public var cameraIcon = BBLBundle.podBundleImage(named: "camera")
    public var videoIcon = BBLBundle.podBundleImage(named: "video")
    public var placeholderIcon = BBLBundle.podBundleImage(named: "insertPhotoMaterial")
...
open class BBLPhotosPickerViewController: UIViewController {
    ...
    @IBOutlet open var albumPopView: BBLAlbumPopView!
    ...
    public weak var delegate: BBLPhotosPickerViewControllerDelegate? = nil
    public weak var logDelegate: BBLPhotosPickerLogDelegate? = nil
    open var selectedAssets = [BBLPHAsset]()
    public var configure = BBLPhotosPickerConfigure()
    public var customDataSouces: BBLPhotopickerDataSourcesProtocol? = nil
    ...
    @objc open var didExceedMaximumNumberOfSelection: ((BBLPhotosPickerViewController) -> Void)? = nil
    @objc open var handleNoAlbumPermissions: ((BBLPhotosPickerViewController) -> Void)? = nil
    ...
    private var focusedCollection: BBLAssetsCollection? = nil
    ...
    private var photoLibrary = BBLPhotoLibrary()
    ...
    deinit {
        //print("deinit BBLPhotosPickerViewController")
    ...
    private func updateUserInterfaceStyle() {
        if #available(iOS 13.0, *) {
            ...
            let image = BBLBundle.podBundleImage(named: "pop_arrow")
            let subImage = BBLBundle.podBundleImage(named: "arrow")
...
// MARK: - UI & UI Action
extension BBLPhotosPickerViewController {
    ...
    @objc open func makeUI() {
        registerNib(nibName: "BBLPhotoCollectionViewCell", bundle: BBLBundle.bundle())
        ...
        self.popArrowImageView.image = BBLBundle.podBundleImage(named: "pop_arrow")
        self.subTitleArrowImageView.image = BBLBundle.podBundleImage(named: "arrow")
    ...
    private func focused(collection: BBLAssetsCollection) {
        func resetRequest() {
    ...
    private func dismiss(done: Bool) {
        ...
        if done {
            ...
            #endif
            self.delegate?.dismissPhotoPicker(withBBLPHAssets: self.selectedAssets)
            shouldDismiss = self.delegate?.shouldDismissPhotoPicker(withBBLPHAssets: self.selectedAssets) ?? true
            self.completionWithBBLPHAssets?(self.selectedAssets)
            #if swift(>=4.1)
...
// MARK: - BBLPhotoLibraryDelegate
extension BBLPhotosPickerViewController: BBLPhotoLibraryDelegate {
    func loadCameraRollCollection(collection: BBLAssetsCollection) {
    ...
    func loadCompleteAllCollection(collections: [BBLAssetsCollection]) {
...
// MARK: - Camera Picker
extension BBLPhotosPickerViewController: UIImagePickerControllerDelegate, UINavigationControllerDelegate {
        ...
        if let image = (info[.originalImage] as? UIImage) {
            ...
            PHPhotoLibrary.shared().performChanges({
                ...
                if success, let `self` = self, let identifier = placeholderAsset?.localIdentifier {
                    ...
                    var result = BBLPHAsset(asset: asset)
...
extension BBLPhotosPickerViewController {
    ...
    private func videoCheck() {
        func play(asset: (IndexPath,BBLPHAsset)) {
        ...
        #if swift(>=4.1)
        let boundAssets = visibleIndexPaths.compactMap{ indexPath -> (IndexPath,BBLPHAsset)? in
            guard let asset = self.focusedCollection?.getBBLAsset(at: indexPath), asset.phAsset?.mediaType == .video else { return nil }
            return (indexPath,asset)
        }
        #else
        let boundAssets = visibleIndexPaths.flatMap{ indexPath -> (IndexPath,BBLPHAsset)? in
            guard let asset = self.focusedCollection?.getBBLAsset(at: indexPath.row),asset.phAsset?.mediaType == .video else { return nil }
            return (indexPath,asset)
        }
        #endif
...
extension BBLPhotosPickerViewController: PHLivePhotoViewDelegate {
    private func stopPlay() {
        guard let playRequest = self.playRequestID else { return }
        self.playRequestID = nil
        guard let cell = self.collectionView.cellForItem(at: playRequest.indexPath) as? BBLPhotoCollectionViewCell else { return }
        cell.stopPlay()
    }
    
    private func playVideo(asset: BBLPHAsset, indexPath: IndexPath) {
        stopPlay()
        guard let phAsset = asset.phAsset else { return }
        if asset.type == .video {
            guard let cell = self.collectionView.cellForItem(at: indexPath) as? BBLPhotoCollectionViewCell else { return }
...
extension BBLPhotosPickerViewController: PHPhotoLibraryChangeObserver {
    ...
    public func photoLibraryDidChange(_ changeInstance: PHChange) {
        ...
        DispatchQueue.main.async {
            ...
            if changes.hasIncrementalChanges, self.configure.groupByFetch == nil {
                ...
                var order = 0
                #if swift(>=4.1)
                self.selectedAssets = self.selectedAssets.enumerated().compactMap({ (offset,asset) -> BBLPHAsset? in
                    var asset = asset
                ...
                #else
                self.selectedAssets = self.selectedAssets.enumerated().flatMap({ (offset,asset) -> BBLPHAsset? in
                    var asset = asset
                    if let phAsset = asset.phAsset, changes.fetchResultAfterChanges.contains(phAsset) {
                        order += 1
                        asset.selectedOrder = order
...
extension BBLPhotosPickerViewController: UICollectionViewDelegate,UICollectionViewDataSource,UICollectionViewDataSourcePrefetching {
    private func getSelectedAssets(_ asset: BBLPHAsset) -> BBLPHAsset? {
    ...
    private func orderUpdateCells() {
        ...
        for indexPath in visibleIndexPaths {
            guard let cell = self.collectionView.cellForItem(at: indexPath) as? BBLPhotoCollectionViewCell else { continue }
            guard let asset = self.focusedCollection?.getBBLAsset(at: indexPath) else { continue }
    ...
    open func collectionView(_ collectionView: UICollectionView, didSelectItemAt indexPath: IndexPath) {
        guard let collection = self.focusedCollection, let cell = self.collectionView.cellForItem(at: indexPath) as? BBLPhotoCollectionViewCell else { return }
    ...
    open func collectionView(_ collectionView: UICollectionView, didEndDisplaying cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        if let cell = cell as? BBLPhotoCollectionViewCell {
    ...
    open func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        func makeCell(nibName: String) -> BBLPhotoCollectionViewCell {
            let cell = collectionView.dequeueReusableCell(withReuseIdentifier: nibName, for: indexPath) as! BBLPhotoCollectionViewCell
            cell.configure = self.configure
            cell.imageView?.image = self.placeholderThumbnail
            cell.liveBadgeImageView?.image = nil
            return cell
        }
        let nibName = self.configure.nibSet?.nibName ?? "BBLPhotoCollectionViewCell"
        ...
        guard let asset = collection.getBBLAsset(at: indexPath) else { return cell }
    ...
    public func collectionView(_ collectionView: UICollectionView, willDisplay cell: UICollectionViewCell, forItemAt indexPath: IndexPath) {
        guard let cell = cell as? BBLPhotoCollectionViewCell else {
            return
        }
        cell.willDisplayCell()
        if self.usedPrefetch, let collection = self.focusedCollection, let asset = collection.getBBLAsset(at: indexPath) {
            if let selectedAsset = getSelectedAssets(asset) {
                cell.selectedAsset = true
                cell.orderLabel?.text = "\(selectedAsset.selectedOrder)"
            }else{
...
extension BBLPhotosPickerViewController: UICollectionViewDelegateFlowLayout {
...
extension BBLPhotosPickerViewController: UITableViewDelegate, UITableViewDataSource {
    //delegate
    ...
    open func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        let cell = tableView.dequeueReusableCell(withIdentifier: "BBLCollectionTableViewCell", for: indexPath) as! BBLCollectionTableViewCell
        ...
        if let phAsset = collection.getAsset(at: collection.useCameraButton ? 1 : 0) {
            ...
            self.photoLibrary.imageAsset(asset: phAsset, size: size, completionBlock: {  (image,complete) in
                DispatchQueue.main.async {
                    if let cell = tableView.cellForRow(at: indexPath) as? BBLCollectionTableViewCell {
...
extension BBLPhotosPickerViewController: UIViewControllerPreviewingDelegate {
    public func previewingContext(_ previewingContext: UIViewControllerPreviewing, viewControllerForLocation location: CGPoint) -> UIViewController? {
        ...
        guard let pressingCell = collectionView.cellForItem(at: pressingIndexPath) as? BBLPhotoCollectionViewCell else { return nil }
    
        previewingContext.sourceRect = pressingCell.frame
        let previewController = BBLAssetPreviewViewController()
    ...
    public func collectionView(_ collectionView: UICollectionView, contextMenuConfigurationForItemAt indexPath: IndexPath, point: CGPoint) -> UIContextMenuConfiguration? {
        guard self.previewAtForceTouch == true else { return nil }
        guard let cell = collectionView.cellForItem(at: indexPath) as? BBLPhotoCollectionViewCell else { return nil }
...
extension BBLPhotosPickerViewController {
    func selectCameraCell(_ cell: BBLPhotoCollectionViewCell) {
    ...
    func toggleSelection(for cell: BBLPhotoCollectionViewCell, at indexPath: IndexPath) {
        guard let collection = focusedCollection, var asset = collection.getBBLAsset(at: indexPath), let phAsset = asset.phAsset else { return }
        ...
        //deselect
            ...
            #if swift(>=4.1)
            selectedAssets = selectedAssets.enumerated().compactMap({ (offset,asset) -> BBLPHAsset? in
                var asset = asset
                asset.selectedOrder = offset + 1
                return asset
            })
            #else
            selectedAssets = selectedAssets.enumerated().flatMap({ (offset,asset) -> BBLPHAsset? in
                var asset = asset
                asset.selectedOrder = offset + 1
                return asset
            })
...
extension BBLPhotosPickerViewController: UINavigationBarDelegate {
