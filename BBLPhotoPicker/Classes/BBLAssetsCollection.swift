...
public struct BBLPHAsset {
    ...
    //Bool to check if BBLPHAsset returned is created using camera.
    ...
    public var fullResolutionImage: UIImage? {
        get {
            ...
            return BBLPhotoLibrary.fullResolutionImageData(asset: phAsset)
        }
    }
    ...
    public func cloudImageDownload(progressBlock: @escaping (Double) -> Void, completionBlock:@escaping (UIImage?)-> Void ) -> PHImageRequestID? {
        guard let phAsset = self.phAsset else { return nil }
        return BBLPhotoLibrary.cloudImageDownload(asset: phAsset, progressBlock: progressBlock, completionBlock: completionBlock)
    }
    ...
    public static func asset(with localIdentifier: String) -> BBLPHAsset? {
        let fetchResult = PHAsset.fetchAssets(withLocalIdentifiers: [localIdentifier], options: nil)
        return BBLPHAsset(asset: fetchResult.firstObject)
    }
}

extension BBLPHAsset: Equatable {
    public static func ==(lhs: BBLPHAsset, rhs: BBLPHAsset) -> Bool {
...
public struct BBLAssetsCollection {
    ...
    public var sections: [(title: String, assets: [BBLPHAsset])]? = nil
    var count: Int {
        get {
    ...
    func getBBLAsset(at indexPath: IndexPath) -> BBLPHAsset? {
        ...
        if let sections = self.sections {
            ...
            guard let result = self.fetchResult, index < result.count else { return nil }
            return BBLPHAsset(asset: result.object(at: max(index,0)))
        }
    }
    ...
    mutating func reloadSection(groupedBy: PHFetchedResultGroupedBy) {
        ...
        if self.useCameraButton {
            groupedSections.insert(("camera",[BBLPHAsset(asset: nil)]), at: 0)
        }
        self.sections = groupedSections
    }
    
    static func ==(lhs: BBLAssetsCollection, rhs: BBLAssetsCollection) -> Bool {
