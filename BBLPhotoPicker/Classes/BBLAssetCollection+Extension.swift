//
//  BBLAssetCollection+Extension.swift
//  BBLPhotoPicker
//
...
extension BBLAssetsCollection {
    func enumarateFetchResult(groupedBy: PHFetchedResultGroupedBy) -> Dictionary<String,[BBLPHAsset]> {
        ...
        let sections = Dictionary(grouping: assets.map{ BBLPHAsset(asset: $0) }) { (element) -> String in
    ...
    func section(groupedBy: PHFetchedResultGroupedBy) -> [(String,[BBLPHAsset])] {
        let dict = enumarateFetchResult(groupedBy: groupedBy)
        var sections = [(String,[BBLPHAsset])]()
