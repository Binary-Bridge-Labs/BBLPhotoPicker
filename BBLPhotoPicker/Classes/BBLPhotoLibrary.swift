...
protocol BBLPhotoLibraryDelegate: AnyObject {
...
class BBLPhotoLibrary {
    
    weak var delegate: BBLPhotoLibraryDelegate? = nil
...
extension BBLPhotoLibrary {
    func getOption(configure: BBLPhotosPickerConfigure) -> PHFetchOptions {
    ...
    func fetchResult(collection: BBLAssetsCollection?, configure: BBLPhotosPickerConfigure) -> PHFetchResult<PHAsset>? {
    ...
    func fetchCollection(configure: BBLPhotosPickerConfigure) {
        self.albums = nil
        ...
        func getAlbum(subType: PHAssetCollectionSubtype, result: inout [BBLAssetsCollection]) {
            ...
            for collection in collections {
                if !result.contains(where: { $0.localIdentifier == collection.localIdentifier }) {
                    var assetsCollection = BBLAssetsCollection(collection: collection)
        ...
        func getSmartAlbum(subType: PHAssetCollectionSubtype,
                           ...
                           result: inout [BBLAssetsCollection])
            -> BBLAssetsCollection?
        {
        ...
        if let fetchCollectionTypes = configure.fetchCollectionTypes {
            DispatchQueue.global(qos: .userInteractive).async { [weak self] in
                var assetCollections = [BBLAssetsCollection]()
