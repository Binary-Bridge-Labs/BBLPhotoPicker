...
[![Version](https://img.shields.io/cocoapods/v/BBLPhotoPicker.svg?style=flat)](http://cocoapods.org/pods/BBLPhotoPicker)
[![License](https://img.shields.io/cocoapods/l/BBLPhotoPicker.svg?style=flat)](http://cocoapods.org/pods/BBLPhotoPicker)
[![Platform](https://img.shields.io/cocoapods/p/BBLPhotoPicker.svg?style=flat)](http://cocoapods.org/pods/BBLPhotoPicker)
![Swift](https://img.shields.io/badge/%20in-swift%205.0-orange.svg)

## Written in Swift 5.0

BBLPhotoPicker enables application to pick images and videos from multiple smart album in iOS, similar to the current facebook app.

## Demo 🙉

| Facebook Picker | BBLPhotoPicker  |
| ------------- | ------------- |
| ![Facebook Picker](Images/facebook_ex.gif)  | ![BBLPhotoPicker](Images/tlphotopicker_ex.gif)  |
...
- adds long press preview to images. ( to @smeshko ) [Preview](https://github.com/tilltue/BBLPhotoPicker/pull/252#issue-362005178)
...
BBLPhotoPicker is available through [CocoaPods](http://cocoapods.org). To install
...
Specify BBLPhotoPicker into your project's Cartfile:

```
github "tilltue/BBLPhotoPicker"
```

### Swift Package Manager

The Swift Package Manager is a tool for automating the distribution of Swift code and is integrated into the swift compiler. It is in early development, but BBLPhotoPicker does support its use on supported platforms.

Once you have your Swift package set up, adding Alamofire as a dependency is as easy as adding it to the dependencies value of your Package.swift.

```
dependencies: [
    .package(url: "https://github.com/tilltue/BBLPhotoPicker.git", .upToNextMajor(from: "2.1.0"))
]
```
...
```swift 
class ViewController: UIViewController,BBLPhotosPickerViewControllerDelegate {
    var selectedAssets = [BBLPHAsset]()
    @IBAction func pickerButtonTap() {
        let viewController = BBLPhotosPickerViewController()
        viewController.delegate = self
        var configure = BBLPhotosPickerConfigure()
        //configure.nibSet = (nibName: "CustomCell_Instagram", bundle: Bundle.main) // If you want use your custom cell..
        self.present(viewController, animated: true, completion: nil)
    }
    //BBLPhotosPickerViewControllerDelegate
    func shouldDismissPhotoPicker(withBBLPHAssets: [BBLPHAsset]) -> Bool {
        // use selected order, fullresolution image
        self.selectedAssets = withBBLPHAssets
	return true
    }
...
```swift
    ...
    init(withBBLPHAssets: (([BBLPHAsset]) -> Void)? = nil, didCancel: ((Void) -> Void)? = nil)
    var canSelectAsset: ((PHAsset) -> Bool)? = nil
    var didExceedMaximumNumberOfSelection: ((BBLPhotosPickerViewController) -> Void)? = nil
    var handleNoAlbumPermissions: ((BBLPhotosPickerViewController) -> Void)? = nil
    var handleNoCameraPermissions: ((BBLPhotosPickerViewController) -> Void)? = nil
...
class ViewController: UIViewController,BBLPhotosPickerViewControllerDelegate {
    var selectedAssets = [BBLPHAsset]()
    @IBAction func pickerButtonTap() {
        let viewController = BBLPhotosPickerViewController(withBBLPHAssets: { [weak self] (assets) in // BBLAssets
...
**Custom Cell**
Custom Cell must subclass BBLPhotoCollectionViewCell
```Swift
class CustomCell_Instagram: BBLPhotoCollectionViewCell {

}

//If you want custom camera cell?
//only used camera cell
[Sample](https://github.com/tilltue/BBLPhotoPicker/blob/master/Example/BBLPhotoPicker/CustomCameraCell.swift)
...
- When you instanciate a `BBLPhotosPickerViewController` subclass, you can pass a closure called `canSelectAsset` to handle the selection according to some rules.  ( or delegate)

```Swift
//use delegate 
public protocol BBLPhotosPickerViewControllerDelegate: class {
    ...
    func canSelectAsset(phAsset: PHAsset) -> Bool
    ...
}

extension UserViewController: BBLPhotosPickerViewControllerDelegate {
    func canSelectAsset(phAsset: PHAsset) -> Bool {
...
//Related issue: https://github.com/tilltue/BBLPhotoPicker/issues/201
//e.g.
...
class CustomPhotoPickerViewController: BBLPhotosPickerViewController {
...
//for log
public protocol BBLPhotosPickerLogDelegate: class {
...
let viewController = BBLPhotosPickerViewController()
viewController.customDataSouces = CustomDataSources() // inherit BBLPhotopickerDataSourcesProtocol
...
Does your organization or project use BBLPhotoPicker? Please let me know by email.

wade.hawk, junhyi.park@gmail.com

## License 

BBLPhotoPicker is available under the MIT license. See the LICENSE file for more info.
